...
const Sign_in = async (req, res, next) => {
  try {
    ...
    if (searchKey && searchValue && verifyKey && verifyValue) {
      const [results] = await pool.execute(`SELECT id, username, display_name, email, email_verified, ${verifyKey} FROM users WHERE ${searchKey} = ? AND exit_date > CURRENT_DATE`,
        [searchValue]);
      if (!results.length) {
      ...
      if (user.email_verified !== 1) {
        ...
        return res.redirect(307, '/sendVerificationLink');
      }
...
const handle_tokens = async (req, res, next) => {
  ...
  if (id && token) {
    try {
      const [results] = await pool.execute('DELETE FROM tokens WHERE id = ?', [id]);
      const expiresAt = new Date();
      expiresAt.setMinutes(expiresAt.getMinutes() + 10);
      const expiresAtFormatted = expiresAt.toISOString();
      const [tokenInsert] = await pool.execute('INSERT INTO tokens (id, verification_token, token_expires_at) VALUES (?, ?, ?)', [id, token, expiresAtFormatted]);
  ...
  .post(async (req, res, next) => {
    ...
    async (req, res, next) => {
      try {
        ...
        const [results] = await pool.execute('SELECT username, email FROM users WHERE (email = ? AND exit_date > CURRENT_DATE) OR (username = ? AND exit_date > CURRENT_DATE)',
          [email, username]);
        if (results.length) {
    ...
    async (req, res, next) => {
      try {
        ...
        const [result] = await pool.execute(
          'INSERT INTO users (username, display_name, password, email, security_question, join_date, exit_date, email_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [username, displayName, password, email, securityQ, new Date(), new Date('9999-12-31'), 0]
        );
        if (!result.affectedRows) {
          throw Object.assign(new Error('There was an error submitting your data, please try again.'), { statusCode: 500 });
        }
        return res.redirect(307, '/Sign_In');  // 307 will keep it as a post
      }
      catch (err) {
        next(err);
      }
  ...
  (req, res, next) => {
    ...
    req.email.html = `
        <div style="font-family: Arial, sans-serif; color: #333;">
          ...
          <a href="http://localhost:3000/verify-email?token=${token}&id=${id}" 
...
router.route('/Forgot_Password')
  ...
  .post(async (req, res, next) => {
    ...
    try {
      ...
      const [results] = await pool.execute(`SELECT id FROM users WHERE email = ? AND username = ? AND exit_date > CURRENT_DATE`,
        [email, username]);
      if (!results.length) {
    ...
    (req, res, next) => {
      ...
      req.email.html = `
        <div style="font-family: Arial, sans-serif; color: #333;">
          ...
          <a href="http://localhost:3000/update-password?token=${token}&id=${id}" 
             style="display: inline-block; padding: 12px 24px; background-color: #007bff; color: #ffffff; text-decoration: none; border-radius: 5px; font-size: 14px; border: none; cursor: pointer; transition: background-color 0.3s ease;">
             Update Password
          </a>
          <p>If you didn't request this, please ignore this email.</p>
...
router.route('/Forgot_User_and_Password')
  ...
  .post((req, res, next) => {
    ...
    try {
      ...
      console.error(err.stack);
      res.status(400).send(err.message);
      return res.redirect('/Forgot_User_and_Password');
    }
  },
    Sign_in,
    log_in
