    ...
    chats.forEach(chat => {
        if (chat) {
            ...
            if (chat.new_messages) {
                current_chat = $(`<div id="chat-${chat.chat_id}" class="chat new_message"></div>`);
            }
            else {
                current_chat = $(`<div id="chat-${chat.chat_id}" class="chat"></div>`);
            }
            const chat_name = $(`<h2>${chat.chat_name}</h2>`);
            const message_container = $(`<div id="message-${chat.message_id}" class="message">`);
